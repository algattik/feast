...
public abstract class CassandraStore {
	...
	/**
	 * Ensures Cassandra is accessible, else throw a RuntimeException. Creates
	 * Cassandra keyspace and table if it does not already exist
	 *
	...
	public static void validateStore(CassandraConfig cassandraConfig) {
		List<InetSocketAddress> contactPoints = Arrays.stream(cassandraConfig.getBootstrapHosts().split(","))
				.map(host -> new InetSocketAddress(host, cassandraConfig.getPort())).collect(Collectors.toList());
		...
		try {
			...
			if (keyspaceMetadata == null) {
				...
				Map<String, Object> replicationOptions = cassandraConfig.getReplicationOptionsMap().entrySet().stream()
						.collect(Collectors.toMap(Entry::getKey, Entry::getValue));
				KeyspaceOptions createKeyspace = SchemaBuilder.createKeyspace(keyspace).ifNotExists().with()
			...
			Create createTable = SchemaBuilder.createTable(keyspace, cassandraConfig.getTableName()).ifNotExists()
			...
			throw new RuntimeException(String.format(
					...
					contactPoints.stream().map(InetSocketAddress::getHostName).collect(Collectors.joining(",")),
					cassandraConfig.getPort()), e);
		}
		cluster.close();
	}
	...
	private static void validateCassandraTable(Session session) {
		try {
			...
			throw new RuntimeException(String.format("Table created does not match the datastax object mapper: %s",
