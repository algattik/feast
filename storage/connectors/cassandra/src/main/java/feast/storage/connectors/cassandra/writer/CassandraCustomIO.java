...
public class CassandraCustomIO {
	...
	private static TupleTag<CassandraMutation> mutationsTag = new TupleTag<CassandraMutation>("mutations") {
	};
	private static TupleTag<FeatureRow> successfulInsertsTag = new TupleTag<FeatureRow>("successfulInserts") {
	};
	private static TupleTag<FailedElement> failedInsertsTupleTag = new TupleTag<FailedElement>("failedInserts") {
	};
	...
	public static Write write(CassandraConfig cassandraConfig, Map<String, FeatureSetSpec> featureSetSpecs) {
	...
	public static class Write extends PTransform<PCollection<FeatureRow>, WriteResult> {
		...
		public WriteResult expand(PCollection<FeatureRow> input) {
	        ...
	        CassandraMutationMapperFactory mapperFactory = new CassandraMutationMapperFactory(CassandraMutation.class);
	        
			PCollectionTuple cassandraWrite = input
					.apply(ParDo.of(new WriteDoFn(featureSetSpecs, cassandraConfig.getDefaultTtl()))
					.withOutputTags(mutationsTag, TupleTagList.of(successfulInsertsTag).and(failedInsertsTupleTag)))
			;

			cassandraWrite.get(mutationsTag)
				            .apply(
	...
	public static class WriteDoFn extends DoFn<FeatureRow, CassandraMutation> {
		...
		public void processElement(ProcessContext context) {
			...
			try {
				...
				Set<String> featureNames = featureSetSpec.getFeaturesList().stream().map(FeatureSpec::getName)
				...
				for (Field field : featureRow.getFieldsList()) {
					if (featureNames.contains(field.getName())) {
						mutations.add(new CassandraMutation(key, field.getName(),
		...
		private FailedElement toFailedElement(FeatureRow featureRow, Exception exception, String jobName) {
			return FailedElement.newBuilder().setJobName(jobName).setTransformName("CassandraCustomIO")
					.setPayload(featureRow.toString()).setErrorMessage(exception.getMessage())
