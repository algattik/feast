...
public class CassandraOnlineRetriever implements OnlineRetriever {
	...
	public CassandraOnlineRetriever(Session session, String keyspace, String tableName, Tracer tracer, Histogram requestLatency) {
		super();
	...
	public List<List<FeatureRow>> getOnlineFeatures(List<EntityRow> entityRows,
		...
		Map<EntityRow, Map<String, Value>> featureValuesMap = entityRows.stream()
		...
		List<FieldValues> fieldValues = featureValuesMap.values().stream()
				.map(valueMap -> FieldValues.newBuilder().putAllFields(valueMap).build()).collect(Collectors.toList());
	...
	List<String> createLookupKeys(List<String> featureSetEntityNames, List<EntityRow> entityRows,
		...
		try (Scope scope = tracer.buildSpan("Cassandra-makeCassandraKeys").startActive(true)) {
			...
			String featureSetId = String.format("%s/%s:%s", fsSpec.getProject(), fsSpec.getName(), fsSpec.getVersion());
	 ...
	 */
	protected void getAndProcessAll(List<String> keys, List<EntityRow> entityRows,
			Map<EntityRow, Map<String, Value>> featureValuesMap, FeatureSetRequest featureSetRequest) {
			...
			for (int i = 0; i < results.size(); i++) {
				...
				while (!queryRows.isExhausted()) {
					...
					FeatureRow featureRow = FeatureRow.newBuilder().addAllFields(fields).setEventTimestamp(Timestamp
							...
							.build();
					Map<String, FeatureReference> featureNames = featureSetRequest.getFeatureReferences().stream()
							.collect(Collectors.toMap(FeatureReference::getName, featureReference -> featureReference));
					featureRow.getFieldsList().stream().filter(field -> featureNames.keySet().contains(field.getName()))
							.forEach(field -> {
	...
	FeatureRow parseResponse(ResultSet resultSet) {
		...
		while (resultSet.getAvailableWithoutFetching() > 0) {
			...
			try {
				fields.add(Field.newBuilder().setName(row.getString("feature"))
						.setValue(Value.parseFrom(ByteBuffer.wrap(row.getBytesUnsafe("value").array()))).build());
			} catch (InvalidProtocolBufferException e) {
						throw new RuntimeException(e);
			}
		}
	...
	private List<ResultSet> sendMultiGet(List<String> keys) {
		try (Scope scope = tracer.buildSpan("Cassandra-sendMultiGet").startActive(true)) {
			...
			try {
				for (String key : keys) {
					results.add(session
							.execute(query.bind(key).enableTracing().setConsistencyLevel(ConsistencyLevel.TWO)));
				}
				return results;
			} catch (Exception e) {
